...
class ConfigTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        (name, name, scenario) for name, scenario in scenario_configs.SCENARIOS.items()
    )
    ...
    def test_bot_used_by_scenario(self, bot):
        used = any(
            bot in scenario.bots for scenario in scenario_configs.SCENARIOS.values()
        )
    ...
    @parameterized.named_parameters(
        (f"{substrate}_{model}", substrate, model) for substrate, model in _models()
    )
    def test_asset_used_by_bot(self, substrate, model):
        used = any(
            bot.substrate == substrate and bot.model == model
