...
class ScenarioWrapperTest(absltest.TestCase):
    def test_scenario(self):
        ...
        substrate.action_spec.return_value = tuple(f"action_spec_{n}" for n in range(4))
        substrate.observation_spec.return_value = tuple(
            immutabledict.immutabledict(ok=f"ok_spec_{n}", not_ok=f"not_ok_spec_{n}")
            for n in range(4)
        )
        substrate.reward_spec.return_value = tuple(f"reward_spec_{n}" for n in range(4))
        ...
        with scenario_factory.Scenario(
            ...
            with mock.patch.object(random, "choices", return_value=["bot_0", "bot_1"]):
        ...
        with self.subTest(name="observation_spec"):
            self.assertEqual(
                observation_spec, [dict(ok="ok_spec_0"), dict(ok="ok_spec_2")]
            )
        ...
        with self.subTest(name="step_timestep"):
            expected = dm_env.transition(
                reward=[11, 31], observation=[dict(ok=11), dict(ok=31)],
            )
