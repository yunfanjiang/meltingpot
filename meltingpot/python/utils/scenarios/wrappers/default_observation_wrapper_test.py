...
class WrapperTest(absltest.TestCase):
    ...
    def test_change(self):
        ...
        with self.subTest("observation_spec"):
            self.assertEqual(
                wrapped.observation_spec(), [{"RGB": RGB_SPEC, TEST_KEY: TEST_SPEC}]
            )
        with self.subTest("reset"):
            np.testing.assert_equal(
                wrapped.reset(),
                dm_env.restart([{"RGB": RGB_VALUE, TEST_KEY: TEST_VALUE}]),
            )
        with self.subTest("observation"):
            np.testing.assert_equal(
                wrapped.step(mock.sentinel.action),
                dm_env.transition(1, [{"RGB": RGB_VALUE, TEST_KEY: TEST_VALUE}]),
            )

    def test_nochange(self):
        env = mock.Mock(spec_set=base.Substrate)
        env.observation_spec.return_value = [{"RGB": RGB_SPEC, TEST_KEY: DEFAULT_SPEC}]
        env.reset.return_value = dm_env.restart(
            [{"RGB": RGB_VALUE, TEST_KEY: DEFAULT_VALUE}]
        )
        env.step.return_value = dm_env.transition(
            1, [{"RGB": RGB_VALUE, TEST_KEY: DEFAULT_VALUE}]
        )
        ...
        with self.subTest("observation_spec"):
            self.assertEqual(
                wrapped.observation_spec(), [{"RGB": RGB_SPEC, TEST_KEY: DEFAULT_SPEC}]
            )
        with self.subTest("reset"):
            np.testing.assert_equal(
                wrapped.reset(),
                dm_env.restart([{"RGB": RGB_VALUE, TEST_KEY: DEFAULT_VALUE}]),
            )
        with self.subTest("observation"):
            np.testing.assert_equal(
                wrapped.step(mock.sentinel.action),
                dm_env.transition(1, [{"RGB": RGB_VALUE, TEST_KEY: DEFAULT_VALUE}]),
            )
