...
class AllObservationsWrapperTest(parameterized.TestCase):
        ...
        expected = [
            {
                ...
                GLOBAL_KEY: {
                    OBSERVATIONS_KEY: {
                        OBSERVATION_1: dm_env.specs.Array(
    ...
    def test_reset(self):
        ...
        env.reset.return_value = dm_env.restart(
            [
                {OBSERVATION_1: np.ones([1]), OBSERVATION_2: np.ones([2]),},
                {OBSERVATION_1: np.ones([1]) * 2, OBSERVATION_2: np.ones([2]) * 2,},
            ]
            ...
            [
                {
                    ...
                    GLOBAL_KEY: {
                        OBSERVATIONS_KEY: {OBSERVATION_1: np.array([[1.0], [2.0]])},
    ...
    def test_step(self):
        ...
        actual = wrapped.step(
            [np.array(3, dtype=np.int32), np.array(4, dtype=np.int32),]
        )
