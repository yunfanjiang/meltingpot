...
def get_transform(x, y, orientation):
    return game_object_utils.Transform(
        position=game_object_utils.Position(x, y), orientation=orientation
    )
...
"""
        ...
        self.assertEqual(
            1, sum([1 if go["name"] == "apple" else 0 for go in game_objects])
        )
        self.assertEqual(
            14, sum([1 if go["name"] == "wall" else 0 for go in game_objects])
        )
        ...
        for go in game_objects:
            if go["name"] == "wall":
                positions.append(
                    game_object_utils.get_first_named_component(go, "Transform")[
                        "kwargs"
                    ]["position"]
                )
    ...
    "components": [
        ...
        },
        {
            ...
            "kwargs": {
                ...
                "actionOrder": ["move"],
                "actionSpec": {"move": {"default": 0, "min": 0, "max": 4},},
            },
        },
    ],
}
    ...
    "components": [
        {
            ...
            "kwargs": {
                ...
                "stateConfigs": [
                    ...
                    {"state": "badgeWait", "groups": ["badgeWaits"]},
                ],
            },
        },
        {"component": "Transform", "kwargs": {"position": (0, 0), "orientation": "N"}},
...
class BuildAvatarObjectsTest(parameterized.TestCase):
    ...
    def test_simple_build(self, num_players):
        ...
        avatars = game_object_utils.build_avatar_objects(
            num_players=num_players, prefabs=prefabs, player_palettes=None,
        )
    ...
    def test_with_palette_build(self):
        ...
        prefabs = {"avatar": AVATAR}
        avatars = game_object_utils.build_avatar_objects(
            num_players=2, prefabs=prefabs, player_palettes=palettes,
        )
        self.assertLen(avatars, 2)
        self.assertEqual(
            game_object_utils.get_first_named_component(avatars[0], "Appearance")[
                "kwargs"
            ]["palettes"][0],
            palettes[0],
        )
        self.assertEqual(
            game_object_utils.get_first_named_component(avatars[1], "Appearance")[
                "kwargs"
            ]["palettes"][0],
            palettes[1],
        )
...
class BuildAvatarBadgesTest(parameterized.TestCase):
    ...
    def test_simple_build(self, num_players):
        ...
        badges = game_object_utils.build_avatar_badges(
            num_players=num_players, prefabs=prefabs, badge_palettes=None,
        )
    ...
    def test_with_palette_build(self):
        ...
        prefabs = {"avatar_badge": BADGE}
        badges = game_object_utils.build_avatar_badges(
            num_players=2, prefabs=prefabs, badge_palettes=palettes,
        )
        self.assertLen(badges, 2)
        self.assertEqual(
            game_object_utils.get_first_named_component(badges[0], "Appearance")[
                "kwargs"
            ]["palettes"][0],
            palettes[0],
        )
        self.assertEqual(
            game_object_utils.get_first_named_component(badges[1], "Appearance")[
                "kwargs"
            ]["palettes"][0],
            palettes[1],
        )
