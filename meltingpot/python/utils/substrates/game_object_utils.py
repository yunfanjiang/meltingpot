...
def get_named_components(
    game_object_config: PrefabConfig, name: str
) -> Sequence[Any]:  # pytype: disable=not-supported-yet
    return [
        component
...
def get_first_named_component(
    game_object_config: PrefabConfig, name: str
) -> Any:  # pytype: disable=not-supported-yet
...
def build_avatar_objects(
    ...
    for idx in range(0, num_players):
        ...
        sprite_name = get_first_named_component(game_object, "Appearance")["kwargs"][
            "spriteNames"
        ][0]
        new_sprite_name = sprite_name + str(lua_index)
        get_first_named_component(game_object, "Appearance")["kwargs"]["spriteNames"][
            0
        ] = new_sprite_name
        ...
        get_first_named_component(game_object, "Avatar")["kwargs"]["index"] = lua_index
...
def build_avatar_badges(
    ...
    for idx in range(0, num_players):
        ...
        sprite_name = get_first_named_component(badge_object, "Appearance")["kwargs"][
            "spriteNames"
        ][0]
        new_sprite_name = sprite_name + str(lua_index)
        get_first_named_component(badge_object, "Appearance")["kwargs"]["spriteNames"][
            0
        ] = new_sprite_name
...
def _create_game_object(prefab: PrefabConfig, transform: Transform) -> PrefabConfig:
