...
def _validate_action(
    action: Mapping[str, np.ndarray], action_spec: Mapping[str, dm_env.specs.Array]
) -> None:
...
def _immutable_action(
    action: Mapping[str, Numeric], action_spec: Mapping[str, dm_env.specs.Array],
...
def _immutable_action_table(
    ...
    return tuple(_immutable_action(action, action_spec) for action in action_table)
...
class Wrapper(base.Wrapper):
    ...
    def step(self, action: Sequence[np.ndarray]):
        """See base class."""
        action = [self._action_table[int(player_action)] for player_action in action]
    ...
    def action_spec(self) -> Sequence[dm_env.specs.DiscreteArray]:
        ...
        spec = dm_env.specs.DiscreteArray(
            num_values=len(self._action_table), name="action"
        )
